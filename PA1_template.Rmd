...
s <- d %>% 
  group_by(date) %>% 
  summarize(
    spd = sum(steps, na.rm = TRUE), 
    mspd = mean(steps, na.rm = TRUE),
    medspd = median(steps, na.rm = TRUE)) 
```
...
- Mean steps per day were `r mean(s$mspd, na.rm = TRUE)`
- Median steps per day were `r median(s$medspd, na.rm = TRUE)`
...
- The largest steps per interval occured on `r s$date[which.max(s$spi)]`
...
- There are `r s[1]` NA values in the steps data.
